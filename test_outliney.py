...
# Make sure 'src' is in your PYTHONPATH or use the sys.path.append() method 
...
if src_path not in sys.path:
    sys.path.insert(0, src_path)  # Use insert(0, ...) to prioritize it
...
# Define the model to be used by the agents (if you plan to use agents later)
...
# Function to load configuration from the selected genre (if needed)
...
# Load the genre configuration (if needed)
genre_config = load_genre_config(GENRE)

# Get the number of chapters from the genre config (if needed)
num_chapters = genre_config.get('NUM_CHAPTERS', 3)

# Get the initial prompt (if you plan to use agents later)
...
yw7File = Yw7File(f'{output_dir}/outline.yw7')  # Use relative path
print(f"Attempting to write to: {yw7File.filePath}")  # Debugging print
yw7File.novel = novel
